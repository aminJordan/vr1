...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ✅ درخواست دسترسی با دوربین پشت
  async function requestCameraAccess() {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: {
          facingMode: { exact: "environment" }, // 🔥 مهم
          width: { ideal: 640 },
          height: { ideal: 480 }
        }
      });
      ...
      console.warn("Back camera not available, fallback...");
      
      // fallback اگر environment نبود
      try {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: true
        });
  ...
  const mindarThree = new MindARThree({
    ...
    videoSettings: {
      facingMode: "environment", // 🔥 این خط خیلی مهمه
      width: { ideal: 640 },
      height: { ideal: 480 }
    }
  });
